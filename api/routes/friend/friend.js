  ...
  async (req, res) => {
    ...
    try {
      ...
      await friendRequest.save((err, result) => {
        ...
        return res
          .status(200)
          .json({ requested_friend_id: requestedFriend._id });
      });
    } catch (error) {
...
router.get('/', auth, async (req, res) => {
  try {
    ...
    let friendsList = await Friend.find({ relating_user: userId })
      .select('acceptance date_added relationship relating_user')
